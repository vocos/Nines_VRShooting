...
public class EnemySpawner : MonoBehaviour {
	...
	void Start () {
		...
		{
			var spawnPosition = new Vector3(UnityEngine.Random.Range(-8f, 8f), 0f, UnityEngine.Random.Range(-8f, 8f));
			var spawnRotation = Quaternion.Euler(0f, UnityEngine.Random.Range(0, 180), 0f);
	...
	{
		Debug.Log (enemiesJSON.enemies.Count);
		foreach (NetworkManager.UserJSON enemyJSON in enemiesJSON.enemies)
		{
			...
			Debug.Log (enemyJSON.health);
			Vector3 position = new Vector3(enemyJSON.position[0], enemyJSON.position[1] - 1, enemyJSON.position[2]);
